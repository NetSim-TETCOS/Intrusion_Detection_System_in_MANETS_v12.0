...
static bool validate_list(NetSim_PACKET* packet)
{
	while (packet)
	{
		if (!fnValidatePacket(packet))
			return false;
...
static void remove_from_list(NetSim_PACKET** list, NetSim_PACKET* curr)
{
	NetSim_PACKET* t = *list;
...
//		assert(validate_list(*pstruList));
